...
QPair<Tag*, Tag*>
HtmlScanner::createTagPairAt(int position)
{
  auto openTag = new HtmlTag();
  openTag->startCursor = createCursor(position);
  openTag->endCursor = createCursor(position + 2);

  auto closeTag = new CloserTag();
  closeTag->startCursor = createCursor(position + 2);
  closeTag->endCursor = createCursor(position + 5);

  return qMakePair(openTag, closeTag);
}

bool
HtmlScanner::xml() const
{
  return m_xml;
}

void
HtmlScanner::setXml(bool Xml)
{
  m_xml = Xml;
}

HtmlTag *HtmlScanner::root() const
{
  return m_root;
}
                                  ...
                                  QTextCursor startCursor)
{
  ...
  if (!(s.toLower() == s))
    xmltag->errors.setFlag(CaseError);
  ...
  for (auto i = startPoint; i < endPoint; i++) {
    ...
    if (c == Characters::LT) {
      ...
      if (expected == Attr) { // After <?xml ... ?> char
        dynamic_cast<struct XmlDeclaration*>(tag)->endChar = storedChar;
      }
      continue;
      ...
      if (!(tag->type == TagXml || tag->type == TagDoctype ||
            tag->type == TagComment)) {
        // Ignore these types for tag tree.
        // These should ALL be either HtmlTag or CloserTag.
        // parentTag has to be an HtmlTag.
        if (!m_root) {

          // this should be an html tag
          auto htmlTag = dynamic_cast<HtmlTag*>(tag);
          m_root = htmlTag;
          parentTag = htmlTag;
          auto name = htmlTag->name;
          if (name != "html") {
            htmlTag->errors.setFlag(RootTagNotHtml);
            if (m_xml && name != name.toLower()) {
              htmlTag->errors.setFlag(CaseError);
            }
          }
        } else {
          auto closerTag = dynamic_cast<CloserTag*>(tag);
          auto htmlTag = dynamic_cast<HtmlTag*>(tag);
          if (closerTag) {
            // TODO possibly move closer tags to same level as their partner.
            parentTag->m_nextLevel.append(closerTag);
            tag->m_parentLevel = parentTag;
            auto pTag = dynamic_cast<HtmlTag*>(parentTag);
            if (pTag && closerTag->name != pTag->name) {
              htmlTag->errors.setFlag(MismatchedTags);
              parentTag = closerTag;
            } else {
              // this might not work correctly ??
              // mismatched tags should not move back down ??
              parentTag = parentTag->m_parentLevel;
            }
          }

          if (htmlTag) {
            if (htmlTag->isClosed()) {
              parentTag->m_nextLevel.append(htmlTag);
              tag->m_parentLevel = parentTag;
            } else {
              // move up a level
              parentTag->m_nextLevel.append(htmlTag);
...
void
HtmlScanner::removeTag(Tag* tag)
{
  int index = m_tagList.indexOf(tag);
