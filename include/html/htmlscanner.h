...
enum ScannerError
{
  ...
  RootTagNotHtml = 0x100000,
  CaseError = 0x200000,
  MismatchedTags = 0x400000,

  //  xml declaration errors
  //  XmlTagNameCaseError = 0x1000000,       //!< xml MUST be lower case
  XmlTagNameError = 0x1000000,          //!< xml MUST be lower case
  XmlTagNamePositionError = 0x2000000,  //!, 'xml' not directly after ?
  XmlTagStartCharError = 0x4000000,     //!< no ? after <
  XmlTagStartPositionError = 0x800000, //!< ? not directly after <
  XmlAttributeError = 0x10000000,
  XmlTagEndError = 0x20000000,
  XmlTagEndPositionError = 0x40000000,
  XmlTagWrongVersion = 0x80000000,
...
struct Tag
{
  ...
  //! up link in Tag tree
  QVector<Tag*> m_nextLevel;
  //! down link in Tag tree
  Tag* m_parentLevel;
...
public:
  ...
  /*!
   *  \brief Replaces the firstTag by the the new secondTag.
   */
  void replaceTag(Tag* firstTag, Tag* secondTag);

  /*!
   *  \brief Removes the Tag.
   */
  ...
  //! Creates a new QTextCursor at the supplied position.
  QTextCursor createCursor(int position);
  //! Creates an empty html tag pair at the supplied position
  QPair<Tag*, Tag*> createTagPairAt(int position);

  bool xml() const;
  void setXml(bool Xml);

  //! Returns a pointer to the Tag tree.
  //!
  //! The tag tree contains a tree of the html and html closer tags. It does
  //! not include comments, xml declaration, doc type declarations or text tags.
  //! A typical tree would be similar to that below.
  //! \code
