...
public:
  ...
  inline  static QVector<QString> voidTagNames() {
    return VOID_HTML5_TAG_NAMES;
  }

  //! \brief A list of all html 5 tag names.
  //!
  //! These are NOT in alphabetical order but start with the most commonly
  //! used tags first.
  inline  static QVector<QString> tagNames() {
    return HTML5_TAG_NAMES;
  }

  //! \brief A list of all html 5 tag names.
  inline  static QVector<QString> orderedTagNames() {
    auto tags = HTML5_TAG_NAMES;
    std::sort(tags.begin(), tags.end());
    return tags;
  }

  inline  static QVector<QString> pseudoClasses() {
    return PSEUDO_CLASSES;
  }

  inline  static QVector<QString> globalAttributes() {
    return GLOBAL_ATTRIBUTES;
  }

  inline  static QVector<QString> eventAttributes() {
    return EVENT_ATTRIBUTES;
  }

  inline static QMap<QString, QMap<QString, QVector<QString>>> tagAttributeMap() {
    return TAG_ATTRIBUTE_MAP;
  }

  inline static QMap<QString, QVector<QString>> attributesForTag(const QString& name) {
    return TAG_ATTRIBUTE_MAP.value(name);
  }
